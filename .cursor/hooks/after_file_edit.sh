#!/bin/bash

# stdinからJSONを受け取る
json_input=$(cat)

# jqを使ってfile_pathを取得
file_path=$(echo "$json_input" | jq -r '.file_path')

# file_pathが空でないことを確認
if [ -z "$file_path" ] || [ "$file_path" = "null" ]; then
    echo "Error: file_path not found in JSON input" >&2
    exit 1
fi

# Pythonファイルかどうかをチェック（拡張子が.pyの場合）
if [[ "$file_path" == *.py ]]; then
    echo "Running linting and type checking for: $file_path"
    
    # ruff check --fix を実行
    uv run ruff check --fix "$file_path"
    ruff_check_exit_code=$?
    
    # ruff format を実行
    uv run ruff format "$file_path"
    ruff_format_exit_code=$?
    
    # mypy を実行
    uv run mypy "$file_path"
    mypy_exit_code=$?
    
    # いずれかのコマンドが失敗した場合はエラーコードを返す
    if [ $ruff_check_exit_code -ne 0 ] || [ $ruff_format_exit_code -ne 0 ] || [ $mypy_exit_code -ne 0 ]; then
        exit 1
    fi
fi
